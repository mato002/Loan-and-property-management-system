...
{
    /**
     * Idempotent repeat of the LoanBook `id` / AUTO_INCREMENT repair (disbursements, collection entries, agents, rates).
     * Use when `2026_04_18_140000_*` already ran before the re-fetch + PRI/UNI key fix, or when fixing by hand failed with #1075.
     *
     * @var list<string>
     */
    private const TABLES = [
        'loan_book_disbursements',
        'loan_book_collection_entries',
        'loan_book_agents',
        'loan_book_collection_rates',
    ];
    ...
    private function repairTableIdAutoIncrement(string $database, string $table): void
    {
        ...
        // Re-read `id` after stripping other columns (MySQL #1075 if another column had AUTO_INCREMENT).
        $idColumn = DB::table('information_schema.COLUMNS')
            ->select(['COLUMN_TYPE', 'COLUMN_KEY', 'EXTRA'])
            ->where('TABLE_SCHEMA', $database)
            ->where('TABLE_NAME', $table)
        ...
        // AUTO_INCREMENT must be on a PRIMARY or UNIQUE key (not plain MUL / unindexed).
