...
/**
 * Creates accounting "books" module tables if they are missing.
 * Use when an older migrate batch was recorded but tables were never created on this database.
 */
...
return new class extends Migration
{
    public function up(): void
    {
        if (! Schema::hasTable('accounting_company_expenses')) {
            Schema::create('accounting_company_expenses', function (Blueprint $table) {
                ...
                $table->timestamps();
                $table->index('expense_date', 'acct_ce_date_idx2');
            });
        }
        ...
        if (! Schema::hasTable('accounting_company_assets')) {
            Schema::create('accounting_company_assets', function (Blueprint $table) {
                ...
                $table->text('notes')->nullable();
                $table->timestamps();
                $table->index('status', 'acct_ca_asset_st2');
            });
        }
        ...
        if (! Schema::hasTable('accounting_payroll_lines')) {
            Schema::create('accounting_payroll_lines', function (Blueprint $table) {
                ...
                $table->text('notes')->nullable();
                $table->timestamps();
                $table->unique(['accounting_payroll_period_id', 'employee_id'], 'acct_pl_pemp_uq2');
            });
        }
        ...
        if (! Schema::hasTable('accounting_budget_lines')) {
            Schema::create('accounting_budget_lines', function (Blueprint $table) {
                ...
                $table->text('notes')->nullable();
                $table->timestamps();
                $table->index(['fiscal_year', 'month'], 'acct_bl_ym_idx2');
            });
        }
        ...
        if (! Schema::hasTable('accounting_bank_reconciliations')) {
            Schema::create('accounting_bank_reconciliations', function (Blueprint $table) {
                ...
                $table->foreign('accounting_chart_account_id', 'acct_br_chart_fk2')
                    ->references('id')
                ...
                $table->text('notes')->nullable();
                $table->timestamps();
                $table->index('statement_date', 'acct_br_stmt_idx2');
            });
        }
    }
    ...
    {
        // Intentionally empty: do not drop tables that may already be in use.
    }
};
