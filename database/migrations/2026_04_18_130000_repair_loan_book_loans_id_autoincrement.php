...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $idColumn = DB::table('information_schema.COLUMNS')
            ...
            ->where('TABLE_NAME', 'loan_book_loans')
        ...
        DB::statement('ALTER TABLE `loan_book_loans` MODIFY `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT');
    }
    ...
    private function stripAutoIncrementFromNonIdColumns(string $database): void
    {
        ...
        foreach ($columns as $col) {
            ...
            $sql = sprintf(
                'ALTER TABLE `loan_book_loans` MODIFY `%s` %s %s%s%s',
                $col->COLUMN_NAME,
                $col->COLUMN_TYPE,
                $nullability,
                $defaultClause,
    ...
    private function ensurePrimaryKeyIsId(string $database): void
    {
        ...
        if ($primaryKeyColumns !== []) {
            DB::statement('ALTER TABLE `loan_book_loans` DROP PRIMARY KEY');
        }

        DB::statement('ALTER TABLE `loan_book_loans` ADD PRIMARY KEY (`id`)');
    }
};
