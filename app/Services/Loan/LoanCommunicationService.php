...
class LoanCommunicationService
{
     ...
     */
    public function sendNow(array $payload, array $recipients): LmMessage
    {
     ...
     * @param  list<string>  $recipients
     */
    public function schedule(array $payload, array $recipients): LmMessage
    {
        ...
        return DB::transaction(function () use ($payload, $recipients, $sendNow) {
            ...
            if ($idempotencyKey !== '') {
                ...
                if ($existing) {
                    if (
                        $sendNow
                        && (string) ($payload['category'] ?? '') === 'payment_reminder'
            ...
            if (($payload['is_bulk'] ?? false) === true) {
                $batch = LmMessageBatch::query()->create([
            ...
            $message = LmMessage::query()->create([
                'batch_id' => $batch?->id,
                'created_by_user_id' => $payload['created_by_user_id'] ?? null,
                'channel' => $channel,
            ...
            if ($recipientRows !== []) {
                LmMessageRecipient::query()->insert($recipientRows);
            }
            ...
            if ($resolvedSendNow) {
                ...
                if (($payload['is_bulk'] ?? false) === true) {
                    SendLoanBulkCommunicationJob::dispatch($message->id);
                } else {
    ...
    public function queueMessageRecipients(LmMessage $message, bool $sync = false): void
    {
        $message->loadMissing('recipients');

        $pendingSms = $message->recipients
            ->where('channel', 'sms')
            ->filter(fn (LmMessageRecipient $recipient) => in_array($recipient->status, ['queued', 'scheduled', 'sending'], true));
        ...
        foreach ($message->recipients as $recipient) {
            ...
            if ($recipient->channel === 'sms') {
                ...
                if ($sync) {
                    ...
                    SendLoanEmailJob::dispatch($recipient->id);
                }
            }
        }
    }

    public function dispatchSmsRecipient(LmMessageRecipient $recipient): void
    {
        ...
        $result = $this->sms->sendNow($message->body, [$recipient->to_address], $message->created_by_user_id, null, 'loan');
    ...
    public function dispatchEmailRecipient(LmMessageRecipient $recipient): void
    {
        $message = $recipient->message;
        if (! $message || $recipient->status === 'sent') {
            return;
    ...
    {
        $delivery = LmMessageDelivery::query()
            ->withoutGlobalScopes()
    ...
    private function startDelivery(LmMessageRecipient $recipient): LmMessageDelivery
    {
        return LmMessageDelivery::query()->create([
    ...
    private function markDeliverySuccess(LmMessageRecipient $recipient, LmMessageDelivery $delivery, array $meta = []): void
    {
        $delivery->update([
        ...
        if ($recipient->channel === 'sms' && ($recipient->message?->category ?? '') === 'payment_reminder') {
    ...
    private function markDeliveryFailure(
        LmMessageRecipient $recipient,
        LmMessageDelivery $delivery,
        string $reason,
    ...
    private function redispatchRentReminderRecipients(LmMessage $message, bool $sync): void
    {
        $message->loadMissing('recipients');
    ...
    private function rentReminderEligibility(): LoanPaymentReminderEligibilityService
    {
        return app(LoanPaymentReminderEligibilityService::class);
    }
    ...
    private function checkRecipientPolicy(string $channel, string $category, string $recipientType, int $recipientId): array
    {
        ...
        $pref = LmMessagePreference::query()
        ...
        if ($category === 'payment_reminder' && ! (bool) $pref->allow_payment_reminders) {
            return ['allowed' => false, 'reason' => 'Recipient opted out of payment reminders.'];
        }
     ...
     */
    private function persistAttachments(LmMessage $message, array $attachments): void
    {
        foreach ($attachments as $item) {
            if ($item instanceof UploadedFile) {
                $storedPath = $item->store('loan/messages', 'public');
                LmMessageAttachment::query()->create([
                    'message_id' => $message->id,
                    'disk' => 'public',
                    'path' => $storedPath,
    ...
    private function syncNoticeDeliveryProof(LmMessageDelivery $delivery, string $state): void
    {
        // Loan module does not attach legal notices to message deliveries.
    }
    ...
    private function evaluateDispatchPolicy(string $channel, int $recipientCount, array $payload = []): array
    {
        ...
        $now = now();
        $startHour = (int) config('loan_communication.sms_send_window_start_hour', 8);
        $endHour = (int) config('loan_communication.sms_send_window_end_hour', 19);
        ...
        $dailyLimit = (int) config('loan_communication.daily_sms_limit', 0);
        if ($dailyLimit > 0) {
            $todaySent = LmMessageRecipient::query()
                ->withoutGlobalScopes()
                ->where('channel', 'sms')
    ...
    private function guardSmsBatchAffordability(LmMessage $message, int $recipientCount): void
    {
        if ($recipientCount < 1) {
            return;
        }
    ...
    private function syncConversationForRecipient(LmMessageRecipient $recipient, string $direction, ?string $error = null): void
    {
        $message = $recipient->message;
        if (! $message) {
            return;
        ...
        $conversation = LmConversation::query()->firstOrCreate(
            [
                'loan_client_id' => $recipient->recipient_type === 'client' ? $recipient->recipient_id : null,
        ...
        LmConversationMessage::query()->create([
    ...
    private function messageLogSentDuplicate(LmMessageRecipient $recipient): bool
    {
    ...
    public function releaseScheduledMessage(LmMessage $message): bool
    {
        $message->loadMissing('recipients');
        ...
        $pending = $message->recipients
            ->filter(fn (LmMessageRecipient $recipient) => in_array($recipient->status, ['scheduled', 'queued'], true));
        ...
        if ($fresh->batch_id !== null) {
            SendLoanBulkCommunicationJob::dispatch((int) $fresh->id);
        } else {
