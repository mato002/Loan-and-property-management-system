    ...
    <x-slot name="above">
        ...
        <div class="property-compact-panel rounded-xl sm:rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-gray-800/80 shadow-sm">
            <x-property.responsive.quick-action-grid>
                <a href="{{ route('property.properties.units', absolute: false) }}" class="quick-action-btn border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700/50">All units</a>
                <a href="{{ route('property.properties.units', array_merge((array) ($filters ?? []), ['status' => 'vacant']), absolute: false) }}" class="quick-action-btn border border-emerald-300 text-emerald-700 hover:bg-emerald-50">Vacant</a>
                <a href="{{ route('property.properties.units', array_merge((array) ($filters ?? []), ['status' => 'occupied']), absolute: false) }}" class="quick-action-btn border border-indigo-300 text-indigo-700 hover:bg-indigo-50">Occupied</a>
                <a href="{{ route('property.properties.units', array_merge((array) ($filters ?? []), ['status' => 'notice']), absolute: false) }}" class="quick-action-btn border border-amber-300 text-amber-700 hover:bg-amber-50">Notice</a>
                <a href="{{ route('property.properties.units.export', request()->query(), false) }}" data-turbo="false" class="quick-action-btn border border-indigo-300 text-indigo-700 hover:bg-indigo-50">Export</a>
            </x-property.responsive.quick-action-grid>
        </div>
        ...
        <div class="rounded-xl sm:rounded-2xl border border-indigo-200 bg-gradient-to-br from-indigo-50 to-white p-3 md:p-5 shadow-sm max-w-3xl">
            <p class="text-base sm:text-lg font-semibold text-slate-900">Setup flow: Units → Tenants → Rent</p>
            <p class="mt-1 text-xs sm:text-sm text-slate-600">Add units (doors) here. Then allocate a vacant unit to a tenant using a Lease, then bill rent using an Invoice.</p>
            <x-property.responsive.quick-action-grid class="mt-3">
                <a href="{{ route('property.tenants.directory', absolute: false) }}" data-turbo-frame="property-main" class="quick-action-btn border border-slate-300 bg-white text-slate-700 hover:bg-slate-50">
                    Add tenant
                    <i class="fa-solid fa-users" aria-hidden="true"></i>
                </a>
                <a href="{{ route('property.tenants.leases', absolute: false) }}" data-turbo-frame="property-main" class="quick-action-btn bg-blue-600 text-white hover:bg-blue-700">
                    Allocate lease
                    <i class="fa-solid fa-arrow-right" aria-hidden="true"></i>
                </a>
                <a href="{{ route('property.listings.vacant', absolute: false) }}" data-turbo-frame="property-main" class="quick-action-btn border border-slate-300 bg-white text-slate-700 hover:bg-slate-50">
                    Publish
