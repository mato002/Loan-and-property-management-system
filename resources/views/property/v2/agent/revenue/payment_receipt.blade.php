@php
    $brandName = \App\Models\PropertyPortalSetting::getValue('company_name', '');
    if ($brandName === '') {
        $brandName = config('app.name', 'Property Management System');
    }
    $logoRaw = trim((string) \App\Models\PropertyPortalSetting::getValue('company_logo_url', ''));
    $logoUrl = null;
    if ($logoRaw !== '') {
        if (str_starts_with($logoRaw, 'http://') || str_starts_with($logoRaw, 'https://') || str_starts_with($logoRaw, '/')) {
            $logoUrl = $logoRaw;
        } else {
            $logoUrl = \Illuminate\Support\Facades\Storage::url($logoRaw);
        }
    }
@endphp
<x-property-layout>
    ...
    >
        ...
        @php
            $allocatedTotal = $allocatedTotal ?? round((float) $payment->allocations->sum('amount'), 2);
            $creditCreated = $creditCreated ?? max(0, round((float) $payment->amount - $allocatedTotal, 2));
        @endphp

        <div class="rounded-2xl border border-emerald-200 dark:border-emerald-800 bg-emerald-50/50 dark:bg-emerald-950/20 p-4 mb-4">
            <p class="text-xs font-semibold uppercase tracking-wide text-emerald-800 dark:text-emerald-300">Payment breakdown</p>
            <div class="mt-3 space-y-2 text-sm">
                <div class="flex justify-between"><span class="text-slate-600">Payment amount</span><span class="font-semibold">KES {{ number_format((float) $payment->amount, 2) }}</span></div>
                <div class="flex justify-between"><span class="text-slate-600">Allocated to invoices</span><span class="font-semibold text-blue-700">KES {{ number_format($allocatedTotal, 2) }}</span></div>
                @if ($creditCreated > 0)
                    <div class="flex justify-between"><span class="text-slate-600">Tenant advance credit</span><span class="font-semibold text-emerald-700">KES {{ number_format($creditCreated, 2) }}</span></div>
                @endif
            </div>
        </div>
