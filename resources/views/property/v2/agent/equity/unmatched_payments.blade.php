<x-property-layout>
    ...
    >
        ...
        <form method="get" class="mb-4 flex flex-wrap items-end gap-3">
            ...
            <div>
                <label class="text-xs text-slate-500">Payer name</label>
                <input type="text" name="name" value="{{ $filters['name'] ?? '' }}" placeholder="Name in SMS message..." class="block min-w-[12rem] rounded-xl border-slate-300 shadow-sm">
            </div>
            <div>
        ...
        <div class="mb-4 flex flex-wrap items-center gap-2">
            <form method="post" action="{{ route('property.equity.unmatched.rematch_all', request()->query()) }}" data-turbo-frame="property-main" data-swal-confirm="Auto re-match all currently filtered unmatched payments?">
                @csrf
                <button
                    type="submit"
            ...
            <table class="w-full min-w-[72rem] text-sm">
                <thead class="bg-slate-50 text-slate-600">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left font-bold">Payer</th>
                        <th class="px-4 py-3 text-left font-bold">Source</th>
                        <th class="px-4 py-3 text-left font-bold min-w-[16rem]">Message</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($items as $item)
                        <tr>
                            ...
                            <td class="px-4 py-3">{{ $item->payer_name ?: '—' }}</td>
                            <td class="px-4 py-3">{{ $item->source_label ?? 'Equity' }}</td>
                            <td class="px-4 py-3 align-top">
                                @if (! empty($item->display_message))
                                    <div class="max-w-xl whitespace-pre-wrap break-words text-xs leading-relaxed text-slate-700">{{ $item->display_message }}</div>
                                @else
                    ...
                    @empty
                        <tr><td colspan="10" class="px-4 py-8 text-center text-slate-500">No unmatched transactions.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
