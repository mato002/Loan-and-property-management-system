    ...
    >
        ...
        <x-property.responsive.quick-action-grid class="mb-4">
            @if (auth()->user()->hasPmPermission('team.users.manage'))
                <a href="{{ route('property.settings.roles', absolute: false) }}" data-turbo-frame="property-main" class="quick-action-btn border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700/50">Property users</a>
            @endif
            <a href="{{ route('property.settings.permissions', absolute: false) }}" data-turbo-frame="property-main" aria-current="page" class="quick-action-btn border bg-blue-600 border-blue-600 text-white">Permissions</a>
            @if (auth()->user()->hasPmPermission('settings.access.manage'))
                <a href="{{ route('property.settings.system_setup.access', absolute: false) }}" data-turbo-frame="property-main" class="quick-action-btn border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700/50">Access control</a>
            @endif
