...
>
    <x-slot name="above">
        ...
        @endif
        @if (is_array(session('bulk_notice_errors')) && count(session('bulk_notice_errors')) > 0)
            <div class="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 max-w-3xl">
                <p class="font-semibold">Some notices were skipped</p>
                <ul class="mt-2 list-disc pl-5 space-y-1">
                    @foreach (session('bulk_notice_errors') as $err)
                        <li>{{ $err }}</li>
                    @endforeach
                </ul>
            </div>
    ...
    <x-slot name="table_actions">
        @if (count($tableRows ?? []) > 0)
            <x-property.bulk-action-bar
                form-id="property-notices-bulk-form"
                :action="route('property.tenants.notices.bulk', absolute: false)"
                confirm="Apply bulk status change to selected notices?"
                :actions="[
                    ['value' => 'pending_approval', 'label' => 'Submit for approval'],
                    ['value' => 'approved', 'label' => 'Approve', 'permission' => 'communications.approve_notice'],
                    ['value' => 'sent', 'label' => 'Mark sent', 'permission' => 'communications.send_legal_notice'],
                    ['value' => 'delivered', 'label' => 'Mark delivered', 'permission' => 'communications.send_legal_notice'],
                    ['value' => 'acknowledged', 'label' => 'Acknowledge', 'permission' => 'communications.send_legal_notice'],
                    ['value' => 'escalated', 'label' => 'Escalate', 'permission' => 'communications.send_legal_notice'],
