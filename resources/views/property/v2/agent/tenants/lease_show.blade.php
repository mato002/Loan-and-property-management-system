<x-property.workspace
    ...
    :stats="[
        ...
        ['label' => 'Carry-forward', 'value' => \App\Services\Property\PropertyMoney::kes((float) ($carryForwardTotal ?? 0)), 'hint' => 'Brought-forward debt'],
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div class="mt-2 text-sm text-slate-700 space-y-1">
                ...
                @php
                    $utilityExpenses = collect($lease->utility_expenses ?? [])->filter(
                        fn ($row) => is_array($row) && ! empty($row['type']) && (float) ($row['amount'] ?? 0) > 0
                    );
                @endphp
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div class="mt-2 text-sm text-slate-700 space-y-2">
                ...
                @if ($openingArrears->isNotEmpty())
                    <p class="font-medium text-slate-800">Carry-forward lines</p>
                    ...
                    @endif
                @else
                    <p><span class="text-slate-500">Carry-forward:</span> —</p>
                @endif
