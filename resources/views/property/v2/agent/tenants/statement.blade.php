    ...
    <x-slot name="actions">
        @if ($canRepairAllocations ?? false)
            ...
            >
                ...
                <button
                    type="submit"
                    class="inline-flex items-center gap-2 rounded-xl border border-amber-300 bg-amber-50 px-3 py-2 text-sm font-medium text-amber-900 hover:bg-amber-100"
                >Repair allocations</button>
            </form>
        @endif
        <button
            type="button"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <div class="mt-2 text-sm text-slate-700 space-y-1">
                    ...
                    @if (($canRepairAllocations ?? false) && (int) ($invoiceSummary['openCount'] ?? 0) > 0)
                        <p class="mt-2 text-xs text-amber-800 rounded-lg border border-amber-200 bg-amber-50 px-2 py-1.5">
                            Payments show on the statement but an invoice still looks unpaid?
                            Use <span class="font-medium">Repair allocations</span> to apply matched M-Pesa amounts to the correct invoice.
                        </p>
                    @endif
                </div>
