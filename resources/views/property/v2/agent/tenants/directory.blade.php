...
>
    @php
        ...
        $showImportFormInitially = (bool) ($openImportModal ?? false)
            || ! empty($lastImportStats ?? null)
            || (is_array($lastImportErrors ?? null) && count($lastImportErrors) > 0);
    @endphp
    <x-slot name="above">
        <div
            x-data="{ showTenantForm: @js($errors->any()), showImportForm: @js($showImportFormInitially) }"
            class="space-y-4"
        >
        <div class="property-compact-panel rounded-xl sm:rounded-2xl border border-indigo-200 bg-gradient-to-br from-indigo-50 to-white shadow-sm max-w-3xl">
            <p class="text-base sm:text-lg font-semibold text-slate-900">Tenant onboarding flow</p>
            <p class="mt-1 text-xs sm:text-sm text-slate-600">Step 1: Add tenant → Step 2: Allocate unit (Lease) → Step 3: Create rent bill (Invoice) → Step 4: Collect payment.</p>
        ...
        @php
            $tenantFormShowOpeningArrears = $errors->hasAny([
                'opening_arrears_items', 'opening_arrears_items.*.type', 'opening_arrears_items.*.period',
                'opening_arrears_items.*.amount', 'opening_arrears_amount', 'opening_arrears_as_of', 'opening_arrears_notes',
            ])
                || count((array) old('opening_arrears_items', [])) > 0
                || (float) old('opening_arrears_amount', 0) > 0
                || trim((string) old('opening_arrears_notes', '')) !== '';
            $tenantFormArrearsItems = array_values((array) old('opening_arrears_items', []));
        @endphp
        <x-property.modal
            show="showTenantForm"
        ...
        <form
            ...
            x-data="{
                showOpeningArrearsSection: @js($tenantFormShowOpeningArrears),
                arrearsItems: @js($tenantFormArrearsItems),
