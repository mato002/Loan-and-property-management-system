<x-property-layout>
    <x-slot name="header">System setup · Property onboarding fields</x-slot>

    <x-property.page
        title="Property onboarding fields"
    ...
    >
        <div class="mb-4 flex flex-wrap gap-2">
            ...
            <a href="{{ route('property.settings.system_setup.property_onboarding_fields') }}" aria-current="page" class="rounded-lg bg-blue-600 px-3 py-1.5 text-sm font-medium text-white">Property onboarding fields</a>
        ...
        <form method="post" action="{{ route('property.settings.system_setup.property_onboarding_fields.store') }}" class="space-y-4" x-data="{
                        ...
                        <template x-for="(row, index) in rows" :key="index">
                            <tr class="border-t border-slate-100 dark:border-slate-700/80">
                                <td class="px-3 py-2 align-top">
                                    <input :name="`fields[${index}][key]`" x-model="row.key" type="text" class="w-40 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-gray-900 px-2 py-1.5 text-sm" placeholder="e.g. city" />
                                </td>
                                <td class="px-3 py-2 align-top">
                                    <input :name="`fields[${index}][label]`" x-model="row.label" type="text" class="w-44 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-gray-900 px-2 py-1.5 text-sm" placeholder="City" />
                                </td>
                                <td class="px-3 py-2 align-top">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <button type="submit" class="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">Save property onboarding fields</button>
            </div>
        </form>
    </x-property.page>
</x-property-layout>
