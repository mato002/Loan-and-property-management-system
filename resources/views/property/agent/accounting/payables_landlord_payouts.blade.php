<x-property.workspace
    ...
    :table-rows="$rows->getCollection()->map(function($r) {
        $item = $r->items->first();
        $landlord = $item ? \App\Models\User::query()->find($item->landlord_id) : null;
        $propertyLabel = $item?->property?->name ?? ($item?->description ? \Illuminate\Support\Str::before($item->description, ' (') : '—');
        $landlordProperty = trim(($landlord?->name ?? '—').' / '.($propertyLabel ?? '—'));
        $actions = '<div class=\'flex flex-wrap gap-2\'>';
        if ($r->status === 'draft') {
            $actions .= '<form method=\'post\' action=\''.route('property.accounting.payables.landlord_payouts.approve', $r).'\' class=\'inline\'>'.csrf_field().'<button type=\'submit\' class=\'text-emerald-700 hover:text-emerald-800\'>Approve</button></form>';
            $actions .= '<form method=\'post\' action=\''.route('property.accounting.payables.landlord_payouts.pay', $r).'\' class=\'inline\' onsubmit=\'return confirm(\"Mark this payout as paid and post to ledger?\")\'>'.csrf_field().'<button type=\'submit\' class=\'text-indigo-700 hover:text-indigo-800\'>Mark paid</button></form>';
...
>
    <x-slot name="actions">
        <a href="{{ route('property.accounting.payables.landlord_settlements') }}" class="inline-flex rounded-xl bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700">New from settlement</a>
    </x-slot>
    <x-slot name="toolbar">
