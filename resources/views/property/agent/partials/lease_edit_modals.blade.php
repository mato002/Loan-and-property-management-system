@php
    $formId = $formId ?? 'lease-edit-form-wrapper';
...
<x-property.modal
    ...
    name="lease-edit-optional-fields"
...
<x-property.modal
    ...
    name="lease-edit-opening-arrears"
    title="Carry-forward details"
    max-width="3xl"
>
    <fieldset form="{{ $formId }}" data-lease-arrears-panel class="min-w-0 border-0 p-0 m-0">
        <input type="hidden" name="carry_forward_panel" value="1" />
        <div id="opening-arrears-edit-wrap" class="space-y-3">
            <button type="button" id="open-arrears-line-modal-edit" class="inline-flex min-h-[44px] items-center gap-2 rounded-lg border border-amber-300 bg-amber-100/70 px-3 py-2 text-sm font-medium text-amber-900 hover:bg-amber-100 dark:border-amber-700 dark:bg-amber-800/20 dark:text-amber-300">
            ...
            <div class="grid gap-2 sm:grid-cols-2">
                <div>
                    ...
                    <input type="number" step="0.01" min="0" name="opening_rent_arrears" value="{{ old('opening_rent_arrears', $existingRentArrears ?? null) }}" placeholder="0.00" class="mt-1 w-full rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-gray-900 text-sm px-3 py-2" />
                ...
                <div>
                    ...
                    <input type="month" name="opening_rent_arrears_period" value="{{ old('opening_rent_arrears_period', $existingRentArrearsPeriod ?? null) }}" class="mt-1 w-full rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-gray-900 text-sm px-3 py-2" />
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="opening_rent_arrears_details" value="{{ old('opening_rent_arrears_details', $existingRentArrearsDetails ?? null) }}" placeholder="e.g. Jan-Mar unpaid rent balance" class="mt-1 w-full rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-gray-900 text-sm px-3 py-2" />
...
>
    ...
    <x-slot name="footer">
        <div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            <button type="button" id="cancel-arrears-line-modal-edit" class="min-h-[44px] rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium" @click="showArrearsLineModal = false">Cancel</button>
            <button type="button" id="save-arrears-line-modal-edit" class="min-h-[44px] rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">Add line</button>
        </div>
    </x-slot>
</x-property.modal>

<x-property.modal
    show="showChargeTypeModal"
    close="showChargeTypeModal = false"
    name="lease-edit-charge-type"
    title="Add charge type"
    max-width="md"
    :z-index="7110"
>
    <p class="text-xs text-slate-500 dark:text-slate-400 -mt-1 mb-3">Create a new charge type for this lease form.</p>
    <div>
    ...
    <x-slot name="footer">
        <div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            <button type="button" id="cancel-charge-type-modal" class="min-h-[44px] rounded-lg border border-slate-300 dark:border-slate-600 px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700/60" @click="showChargeTypeModal = false">Cancel</button>
            <button type="button" id="save-charge-type-modal" class="min-h-[44px] rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">Add type</button>
        </div>
    </x-slot>
</x-property.modal>
