@if ($canManage ?? false)
    <div class="property-compact-panel rounded-xl sm:rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-gray-800/80 p-4 sm:p-5 shadow-sm max-w-2xl">
        ...
        @else
            <form method="post" action="{{ route('property.field_officers.properties.assign', $fieldOfficer, false) }}" class="mt-3 flex flex-col sm:flex-row gap-2 items-stretch sm:items-end">
                @csrf
                    ...
                    <td class="px-3 sm:px-4 py-3">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            @if ($canManage ?? false)
                                <form
                                    method="post"
                                    action="{{ route('property.field_officers.properties.detach', $fieldOfficer, false) }}"
                                    data-swal-title="Unassign property?"
                                    data-swal-confirm="Remove this property from {{ $fieldOfficer->name }}?"
