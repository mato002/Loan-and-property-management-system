...
<x-property.workspace
    ...
    subtitle="All overdue invoices for this tenant. Send targeted reminders or escalate."
    back-route="property.revenue.arrears"
    :columns="$columns"
    :table-rows="$tableRows"
    empty-title="No outstanding invoices"
    empty-hint="This tenant has no overdue invoices right now."
>
    <x-slot name="actions">
        <div class="flex flex-wrap items-end gap-2">
            <form id="arrears-reminder-form" method="post" action="{{ route('property.revenue.arrears.reminders', absolute: false) }}" class="flex flex-wrap items-end gap-2">
                ...
                <div>
                    ...
                    <select id="arrears-target-mode" name="target_mode" class="mt-1 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm">
                        <option value="all" @selected(old('target_mode', 'selected') === 'all')>All overdue (this tenant)</option>
            ...
            <div class="flex items-end gap-2">
                <button type="button" id="arrears-select-all" class="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Select all</button>
                <button type="button" id="arrears-clear" class="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Clear</button>
            </div>
        </div>
        ...
        <div class="flex flex-wrap items-stretch gap-3 w-full">
            <div class="flex-1 min-w-[260px] rounded-xl border border-slate-200 bg-white px-4 py-3 dark:border-slate-700 dark:bg-gray-800">
                ...
                <div class="mt-1 flex flex-wrap gap-3 text-xs text-slate-600 dark:text-slate-400">
                    @if ($tenantPhoneE164 !== '')
                        <a href="tel:{{ $tenantPhoneE164 }}" class="text-indigo-600 hover:text-indigo-700">≡ƒô₧ {{ $tenant->phone }}</a>
                    @else
                        <span>≡ƒô₧ —</span>
                    @endif
                    @if (! empty($tenant->email))
                        <a href="mailto:{{ $tenant->email }}" class="text-indigo-600 hover:text-indigo-700">Γ£ë {{ $tenant->email }}</a>
                    @endif
            ...
            <div class="rounded-xl border border-slate-200 bg-white px-4 py-3 dark:border-slate-700 dark:bg-gray-800">
                ...
                <p class="text-xs text-slate-500 mt-1">{{ $summary['invoice_count'] ?? 0 }} overdue invoice(s)</p>
            </div>
            <div class="rounded-xl border border-slate-200 bg-white px-4 py-3 dark:border-slate-700 dark:bg-gray-800">
                ...
                <p class="text-xs text-slate-500 mt-1">{{ $summary['days_late'] ?? 0 }} days ┬╖ {{ $summary['workflow'] ?? 'Reminder' }}</p>
            </div>
            <div class="rounded-xl border border-slate-200 bg-white px-4 py-3 dark:border-slate-700 dark:bg-gray-800">
    ...
    <script>
        (function () {
            ...
            const toggleSingle = () => {
                ...
                if (btn) {
                    btn.textContent = mode.value === 'all'
                        ? "Send to all of this tenant's overdue"
            ...
            form.addEventListener('submit', function () {
                if (mode.value === 'all') {
                    // "All overdue" on the detail page means "all overdue for THIS tenant",
