<x-loan-layout>
    ...
    >
        ...
        <div class="grid max-w-7xl grid-cols-1 items-start gap-6 xl:grid-cols-[minmax(0,1fr)_320px]" x-data="clientFormPreview()">
            ...
            <form method="post" action="{{ route('loan.clients.update', $loan_client) }}" class="space-y-5" enctype="multipart/form-data">
                @csrf
                @method('patch')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <select id="gender" name="gender" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            @foreach (['male' => 'Male', 'female' => 'Female', 'other' => 'Other'] as $value => $label)
                                <option value="{{ $value }}" @selected(old('gender', $loan_client->gender) === $value)>{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <x-text-input id="next_of_kin_contact" name="next_of_kin_contact" type="text" class="mt-1 block w-full" :value="old('next_of_kin_contact', $loan_client->next_of_kin_contact)" />
                    ...
                    <div>
                        ...
                        <x-text-input id="next_of_kin_name" name="next_of_kin_name" type="text" class="mt-1 block w-full" :value="old('next_of_kin_name', $loan_client->next_of_kin_name)" />
                    ...
                    <div>
                        ...
                        @if ($loan_client->client_photo_path)
                            <a href="{{ \Illuminate\Support\Facades\Storage::url($loan_client->client_photo_path) }}" target="_blank" class="mt-1 inline-block text-xs text-indigo-600 hover:text-indigo-500">View current file</a>
                        @endif
                    ...
                    <div>
                        ...
                        @if ($loan_client->id_back_photo_path)
                            <a href="{{ \Illuminate\Support\Facades\Storage::url($loan_client->id_back_photo_path) }}" target="_blank" class="mt-1 inline-block text-xs text-indigo-600 hover:text-indigo-500">View current file</a>
                        @endif
                    ...
                    <div class="sm:col-span-2">
                        ...
                        @if ($loan_client->id_front_photo_path)
                            <a href="{{ \Illuminate\Support\Facades\Storage::url($loan_client->id_front_photo_path) }}" target="_blank" class="mt-1 inline-block text-xs text-indigo-600 hover:text-indigo-500">View current file</a>
                        @endif
                    ...
                    @foreach (($biodataDynamicFields ?? []) as $field)
                        ...
                        @php($fieldValue = old('biodata_meta.'.$fieldKey, data_get($loan_client->biodata_meta, $fieldKey)))
                        @if ($fieldKey !== '')
                            <div class="{{ $fieldType === 'long_text' ? 'sm:col-span-2' : '' }}">
                                ...
                                @if ($fieldType === 'select')
                                    <select id="biodata_{{ $fieldKey }}" name="biodata_meta[{{ $fieldKey }}]" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                        ...
                                        @foreach ($fieldOptions as $opt)
                                            <option value="{{ $opt }}" @selected((string) $fieldValue === (string) $opt)>{{ $opt }}</option>
                                        @endforeach
                                    </select>
                                @elseif ($fieldType === 'long_text')
                                    <textarea id="biodata_{{ $fieldKey }}" name="biodata_meta[{{ $fieldKey }}]" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ (string) $fieldValue }}</textarea>
                                @elseif ($fieldType === 'image')
                                    @if (filled($fieldValue))
                                        <a href="{{ \Illuminate\Support\Facades\Storage::url((string) $fieldValue) }}" target="_blank" class="mt-1 inline-block text-xs text-indigo-600 hover:text-indigo-500">View current file</a>
                                    @endif
                                ...
                                @else
                                    <x-text-input id="biodata_{{ $fieldKey }}" :name="'biodata_meta['.$fieldKey.']'" :type="$fieldType === 'number' ? 'number' : 'text'" class="mt-1 block w-full" :value="(string) $fieldValue" />
                                @endif
            ...
            <aside class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm xl:sticky xl:top-4" x-data="{ mobilePreviewOpen: false }">
                ...
                <div class="mt-3 space-y-1 text-xs text-slate-700" x-show="mobilePreviewOpen || window.innerWidth >= 1280" x-cloak>
                    <template x-if="previewFields.length === 0 && previewImages.length === 0">
                        <p class="text-slate-500">Update fields to preview current draft here.</p>
                    </template>
