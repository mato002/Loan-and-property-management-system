<x-loan-layout>
    <x-loan.page
        ...
        subtitle="Record outreach against a lead or client."
    >
        <x-slot name="actions">
        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-6 sm:p-8 max-w-2xl">
            <form method="post" action="{{ route('loan.clients.interactions.store') }}" class="space-y-5">
                @csrf
                <div>
                    <x-input-label for="loan_client_id" value="Lead or client" />
                    <select id="loan_client_id" name="loan_client_id" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        @foreach ($people as $p)
                            <option value="{{ $p->id }}" @selected(old('loan_client_id') == $p->id)>{{ strtoupper($p->kind) }} · {{ $p->client_number }} — {{ $p->full_name }}</option>
                        @endforeach
                    </select>
