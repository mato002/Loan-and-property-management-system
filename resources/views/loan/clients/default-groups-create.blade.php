<x-loan-layout>
    <x-loan.page
        ...
        subtitle="Define a group and choose contacts before saving."
    >
        <x-slot name="actions">
        ...
        <div
            x-data="{
                showPicker: false,
                search: '',
                selected: @js(collect(old('loan_client_ids', []))->map(fn ($id) => (int) $id)->values()->all()),
                get allSelected() {
                    return this.selected.length > 0 && this.selected.length === this.filteredClients.length;
                },
                get filteredClients() {
            ...
            class="bg-white border border-slate-200 rounded-xl shadow-sm p-6 sm:p-8 max-w-4xl space-y-5"
        >
            <form method="post" action="{{ route('loan.clients.default_groups.store') }}" class="space-y-5">
                @csrf
                <div>
                ...
                <div class="border-y border-slate-200 py-3">
                    ...
                    <x-input-error class="mt-2" :messages="$errors->get('loan_client_ids')" />
                </div>
...
@php
    $groupClientOptions = ($clientOptions ?? collect())
        ->map(function ($c) {
            return [
                'id' => (int) $c->id,
...
@endphp
<script>
    window.__groupClientOptions = @js($groupClientOptions);
</script>
