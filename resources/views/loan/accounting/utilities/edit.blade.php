<x-loan-layout>
    <x-loan.page title="Edit utility payment" subtitle="">
        <x-slot name="actions">
        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-sm max-w-xl overflow-hidden">
            <form method="post" action="{{ route('loan.accounting.utilities.update', $row) }}" class="px-5 py-6 space-y-4">
                @csrf
                @method('patch')
                <div>
                    <label for="utility_type" class="block text-xs font-semibold text-slate-600 mb-1">Utility type</label>
                    <input id="utility_type" name="utility_type" value="{{ old('utility_type', $row->utility_type) }}" required class="w-full rounded-lg border-slate-200 text-sm" />
                ...
                <div>
                    ...
                    <input id="provider" name="provider" value="{{ old('provider', $row->provider) }}" class="w-full rounded-lg border-slate-200 text-sm" />
                ...
                <div>
                    ...
                    <input id="bill_account_ref" name="bill_account_ref" value="{{ old('bill_account_ref', $row->bill_account_ref) }}" class="w-full rounded-lg border-slate-200 text-sm font-mono" />
                ...
                <div>
                    ...
                    <input id="paid_on" name="paid_on" type="date" value="{{ old('paid_on', $row->paid_on->toDateString()) }}" required class="w-full rounded-lg border-slate-200 text-sm" />
                ...
                <div>
                    ...
                    <select id="payment_method" name="payment_method" required class="w-full rounded-lg border-slate-200 text-sm">
                        @foreach (['bank' => 'Bank / EFT', 'mpesa' => 'M-Pesa', 'cash' => 'Cash', 'cheque' => 'Cheque'] as $v => $lab)
                            <option value="{{ $v }}" @selected(old('payment_method', $row->payment_method) === $v)>{{ $lab }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input id="reference" name="reference" value="{{ old('reference', $row->reference) }}" class="w-full rounded-lg border-slate-200 text-sm font-mono" />
