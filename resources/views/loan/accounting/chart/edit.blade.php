<x-loan-layout>
    <x-loan.page title="Edit account {{ $account->code }}" subtitle="">
        <x-slot name="actions">
        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-sm max-w-lg overflow-hidden">
            <form method="post" action="{{ route('loan.accounting.chart.update', $account) }}" class="px-5 py-6 space-y-4">
                @csrf
                @method('patch')
                <div>
                ...
                <div>
                    ...
                    <select id="income_statement_category" name="income_statement_category" class="w-full rounded-lg border-slate-200 text-sm">
                        ...
                        <option value="revenue" @selected(old('income_statement_category', $account->income_statement_category) === 'revenue')>Revenue</option>
                        <option value="direct_cost" @selected(old('income_statement_category', $account->income_statement_category) === 'direct_cost')>Direct Cost</option>
                        <option value="operating_expense" @selected(old('income_statement_category', $account->income_statement_category) === 'operating_expense')>Operating Expense</option>
                        <option value="tax_expense" @selected(old('income_statement_category', $account->income_statement_category) === 'tax_expense')>Tax Expense</option>
                        <option value="other_income" @selected(old('income_statement_category', $account->income_statement_category) === 'other_income')>Other Income</option>
                        <option value="other_expense" @selected(old('income_statement_category', $account->income_statement_category) === 'other_expense')>Other Expense</option>
                    </select>
                ...
                <div>
                    ...
                    <select id="account_class" name="account_class" class="w-full rounded-lg border-slate-200 text-sm">
                        @foreach (['Header', 'Detail'] as $class)
                            <option value="{{ $class }}" @selected(old('account_class', $account->account_class ?? 'Detail') === $class)>{{ $class }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="parent_id" name="parent_id" class="w-full rounded-lg border-slate-200 text-sm">
                        ...
                        @foreach (($headerAccounts ?? collect()) as $header)
                            <option value="{{ $header->id }}" @selected((string) old('parent_id', $account->parent_id) === (string) $header->id)>{{ $header->code }} - {{ $header->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input id="current_balance" type="number" step="0.01" min="0" name="current_balance" value="{{ old('current_balance', $account->current_balance ?? 0) }}" class="w-full rounded-lg border-slate-200 text-sm" />
                ...
                <label class="flex items-center gap-2 text-sm text-slate-700">
                    ...
                    <input type="checkbox" name="allow_overdraft" value="1" class="rounded border-slate-300" @checked(old('allow_overdraft', $account->allow_overdraft)) />
                    Allow overdraft
                </label>
                <div>
                    <label for="overdraft_limit" class="block text-xs font-semibold text-slate-600 mb-1">Overdraft Limit</label>
                    <input id="overdraft_limit" type="number" step="0.01" min="0" name="overdraft_limit" value="{{ old('overdraft_limit', $account->overdraft_limit) }}" class="w-full rounded-lg border-slate-200 text-sm" placeholder="Leave blank for unlimited when enabled" />
                    @error('overdraft_limit')<p class="text-xs text-red-600 mt-1">{{ $message }}</p>@enderror
                </div>
                <label class="flex items-center gap-2 text-sm text-slate-700">
                    <input type="hidden" name="is_cash_account" value="0" />
                    <input type="checkbox" name="is_cash_account" value="1" class="rounded border-slate-300" @checked(old('is_cash_account', $account->is_cash_account)) />
                    Cash / bank account
                </label>
                <label class="flex items-center gap-2 text-sm text-slate-700">
                    <input type="hidden" name="is_active" value="0" />
                    <input type="checkbox" name="is_active" value="1" class="rounded border-slate-300" @checked(old('is_active', $account->is_active)) />
                    Active
                </label>
