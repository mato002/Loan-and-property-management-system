<x-loan-layout>
    <x-loan.page :title="$title" :subtitle="$subtitle">
        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden max-w-3xl">
            <form method="post" action="{{ route('loan.financial.mpesa_payouts.store') }}" class="px-5 py-6 space-y-4">
                @csrf
                <div>
                    <label for="reference" class="block text-xs font-semibold text-slate-600 mb-1">Reference <span class="text-slate-400 font-normal">(optional)</span></label>
                    <input id="reference" name="reference" value="{{ old('reference') }}" placeholder="Leave blank to auto-generate" class="w-full rounded-lg border-slate-200 text-sm" />
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input id="total_amount" name="total_amount" type="number" step="0.01" min="0.01" value="{{ old('total_amount') }}" required class="w-full rounded-lg border-slate-200 text-sm tabular-nums" />
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="3" class="w-full rounded-lg border-slate-200 text-sm">{{ old('notes') }}</textarea>
