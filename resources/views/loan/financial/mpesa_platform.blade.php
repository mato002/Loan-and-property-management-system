    ...
    <x-loan.page :title="$title" :subtitle="$subtitle">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-5">
                ...
                <p class="text-2xl font-bold text-slate-900 tabular-nums mt-2">{{ number_format((float) $stkTodaySum, 2) }}</p>
                <p class="text-xs text-slate-500 mt-1">Channel: STK push</p>
            </div>
            <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-5">
                <p class="text-xs font-semibold text-slate-500 uppercase tracking-wide">C2B (24h sum)</p>
                <p class="text-2xl font-bold text-slate-900 tabular-nums mt-2">{{ number_format((float) $c2b24hSum, 2) }}</p>
                <p class="text-xs text-slate-500 mt-1">Rolling window</p>
            </div>
            <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-5">
                <p class="text-xs font-semibold text-slate-500 uppercase tracking-wide">Failed (all time)</p>
                <p class="text-2xl font-bold text-slate-900 tabular-nums mt-2">{{ $failedCount }}</p>
        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b border-slate-100 flex justify-between items-center">
                ...
                <p class="text-xs text-slate-500">{{ $transactions->total() }} total</p>
            </div>
