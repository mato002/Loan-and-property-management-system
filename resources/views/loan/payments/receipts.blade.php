<x-loan-layout>
    <x-loan.page
        title="Receipts"
        subtitle="Payments that include an M-Pesa receipt number."
    >
        ...
        <form method="get" class="mb-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <div class="flex flex-wrap items-end gap-2">
                <div>
                    ...
                    <input type="text" name="q" value="{{ $q ?? '' }}" placeholder="Ref, receipt, loan, client..." oninput="window.clearTimeout(this._autoSearchTimer); this._autoSearchTimer = window.setTimeout(() => this.form.requestSubmit(), 1100);" class="h-10 w-72 rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-700 shadow-sm">
                </div>
                <div>
                ...
                <a href="{{ route('loan.payments.receipts') }}" class="inline-flex h-10 items-center rounded-lg border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50">Reset</a>
        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <h2 class="text-sm font-semibold text-slate-700">With receipt</h2>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full border-collapse text-sm [&_th]:border [&_th]:border-slate-200 [&_td]:border [&_td]:border-slate-200">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-5 py-12 text-center text-slate-500">No payments with M-Pesa receipts.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
