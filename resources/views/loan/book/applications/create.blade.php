<x-loan-layout>
    <x-loan.page :title="$title" :subtitle="$subtitle">
        @php
            ...
            $mappedBookingSegments = $mappedBookingFieldDisplaySegments ?? [];
        @endphp
        <x-slot name="actions">
        ...
        >
            ...
            <form method="post" action="{{ route('loan.book.applications.store') }}" enctype="multipart/form-data" class="rounded-xl border border-slate-200 bg-white px-5 py-6 shadow-sm space-y-4">
                ...
                <div class="space-y-4">
                    @foreach ($mappedBookingSegments as $segment)
                        @if (($segment['type'] ?? '') === 'grid')
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                @foreach ($segment['keys'] ?? [] as $fieldKey)
                                    <div @class(['sm:col-span-2' => $fieldKey === 'interest_rate_period'])>
                                        @include('loan.book.applications.partials.mapped-booking-field', ['fieldKey' => $fieldKey, 'mapped' => $mapped])
                                    </div>
                                @endforeach
                            </div>
                        @elseif (($segment['type'] ?? '') === 'single')
                            @include('loan.book.applications.partials.mapped-booking-field', ['fieldKey' => $segment['key'], 'mapped' => $mapped])
                        @endif
                    @endforeach
                </div>
    ...
    function loanProductPicker(productMetaByName = {}, initialTermUnit = '', initialSuspensePaymentId = '', pendingDraftCount = 0, productFieldConfig = {}) {
        ...
        return {
            ...
            fieldWrapperHasValue(el) {
                const controls = el.querySelectorAll('input:not([type="hidden"]), select, textarea');
            ...
            applyProductFieldVisibility() {
                ...
                form.querySelectorAll('[data-field-key]').forEach((el) => {
                    ...
                    el.querySelectorAll('[data-required-star]').forEach((star) => {
                        star.classList.toggle('hidden', !needReq);
                    });
