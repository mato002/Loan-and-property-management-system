        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden max-w-2xl">
            <form method="post" action="{{ route('loan.book.applications.store') }}" class="px-5 py-6 space-y-4">
                ...
                <div>
                    ...
                    <select id="loan_client_id" name="loan_client_id" required class="w-full rounded-lg border-slate-200 text-sm">
                        ...
                        @foreach ($clients as $c)
                            <option value="{{ $c->id }}" @selected((string) old('loan_client_id', $selectedClientId ?? '') === (string) $c->id)>{{ $c->full_name }} · {{ $c->client_number }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input id="product_name" name="product_name" value="{{ old('product_name', $defaultProductName ?? '') }}" required class="w-full rounded-lg border-slate-200 text-sm" placeholder="e.g. SME term, Salary advance" />
