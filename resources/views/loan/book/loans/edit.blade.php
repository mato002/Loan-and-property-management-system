        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden max-w-2xl">
            <form method="post" action="{{ route('loan.book.loans.update', $loan) }}" class="px-5 py-6 space-y-4">
                ...
                <div>
                    ...
                    <select id="loan_book_application_id" name="loan_book_application_id" class="w-full rounded-lg border-slate-200 text-sm">
                        ...
                        @foreach ($applications as $a)
                            <option value="{{ $a->id }}" @selected(old('loan_book_application_id', $loan->loan_book_application_id) == $a->id)>{{ $a->reference }} · {{ $a->loanClient->full_name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="loan_client_id" name="loan_client_id" required class="w-full rounded-lg border-slate-200 text-sm">
                        @foreach ($clients as $c)
                            <option value="{{ $c->id }}" @selected(old('loan_client_id', $loan->loan_client_id) == $c->id)>{{ $c->full_name }} · {{ $c->client_number }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input id="product_name" name="product_name" value="{{ old('product_name', $loan->product_name) }}" required class="w-full rounded-lg border-slate-200 text-sm" />
                ...
                <div class="flex items-center gap-2">
                    <input id="is_checkoff" name="is_checkoff" type="checkbox" value="1" class="rounded border-slate-300 text-[#2f4f4f] focus:ring-[#2f4f4f]" @checked(old('is_checkoff', $loan->is_checkoff)) />
                    <label for="is_checkoff" class="text-sm text-slate-700">Checkoff / employer deduct</label>
                </div>
                <div>
                    <label for="checkoff_employer" class="block text-xs font-semibold text-slate-600 mb-1">Employer</label>
                    <input id="checkoff_employer" name="checkoff_employer" value="{{ old('checkoff_employer', $loan->checkoff_employer) }}" class="w-full rounded-lg border-slate-200 text-sm" />
                    ...
                    <div>
                        ...
                        <select id="loan_branch_id" name="loan_branch_id" class="w-full rounded-lg border-slate-200 text-sm">
                            ...
                            @foreach ($branches as $b)
                                <option value="{{ $b->id }}" @selected(old('loan_branch_id', $loan->loan_branch_id) == $b->id)>{{ $b->name }}@if ($b->region) — {{ $b->region->name }}@endif</option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <input id="branch" name="branch" value="{{ old('branch', $loan->branch) }}" class="w-full rounded-lg border-slate-200 text-sm" placeholder="Updated from directory when you pick a branch and save" />
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="2" class="w-full rounded-lg border-slate-200 text-sm">{{ old('notes', $loan->notes) }}</textarea>
                    @error('notes')<p class="text-xs text-red-600 mt-1">{{ $message }}</p>@enderror
                </div>
                <div class="flex flex-wrap gap-2 pt-2">
                    <button type="submit" class="inline-flex items-center justify-center rounded-lg bg-[#2f4f4f] px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-[#264040] transition-colors">Update loan</button>
                </div>
            </form>
        </div>
    </x-loan.page>
