<x-loan-layout>
    <x-loan.page title="Add performance snapshot" subtitle="Manual KPI row until feeds from LoanBook are connected.">
        <x-slot name="actions">
        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-sm p-6 sm:p-8 max-w-xl">
            <form method="post" action="{{ route('loan.analytics.performance.store') }}" class="space-y-5">
                @csrf
                <div>
                ...
                <div>
                    ...
                    <x-text-input id="branch" name="branch" type="text" class="mt-1 block w-full" :value="old('branch')" placeholder="Leave blank for group-wide" />
                    <x-input-error class="mt-2" :messages="$errors->get('branch')" />
                </div>
                <div>
                    <x-input-label for="total_outstanding" value="Total outstanding (Ksh)" />
                    <x-text-input id="total_outstanding" name="total_outstanding" type="number" step="0.01" min="0" class="mt-1 block w-full" :value="old('total_outstanding')" />
                ...
                <div>
                    ...
                    <x-text-input id="disbursements_period" name="disbursements_period" type="number" step="0.01" min="0" class="mt-1 block w-full" :value="old('disbursements_period')" />
                ...
                <div>
                    ...
                    <x-text-input id="collections_period" name="collections_period" type="number" step="0.01" min="0" class="mt-1 block w-full" :value="old('collections_period')" />
                    <x-input-error class="mt-2" :messages="$errors->get('collections_period')" />
                </div>
                <div>
                    <x-input-label for="npl_rate" value="NPL rate (%)" />
                    <x-text-input id="npl_rate" name="npl_rate" type="number" step="0.01" min="0" max="100" class="mt-1 block w-full" :value="old('npl_rate')" />
                ...
                <div>
                    ...
                    <x-text-input id="active_borrowers_count" name="active_borrowers_count" type="number" min="0" class="mt-1 block w-full" :value="old('active_borrowers_count')" />
                    <x-input-error class="mt-2" :messages="$errors->get('active_borrowers_count')" />
                </div>
                <div>
                    <x-input-label for="notes" value="Notes" />
                    <textarea id="notes" name="notes" rows="2" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">{{ old('notes') }}</textarea>
                    <x-input-error class="mt-2" :messages="$errors->get('notes')" />
                </div>
                <x-primary-button>{{ __('Save snapshot') }}</x-primary-button>
            </form>
        </div>
    </x-loan.page>
</x-loan-layout>
