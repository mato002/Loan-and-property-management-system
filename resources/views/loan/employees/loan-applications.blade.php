<x-loan-layout>
    ...
    >
        ...
        <form method="get" class="mb-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <div class="flex flex-wrap items-end gap-2">
                <div>
                    ...
                    <input type="text" name="q" value="{{ $q ?? '' }}" placeholder="Ref, employee, product..." oninput="window.clearTimeout(this._autoSearchTimer); this._autoSearchTimer = window.setTimeout(() => this.form.requestSubmit(), 1100);" class="h-10 w-72 rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-700 shadow-sm focus:border-[#2f4f4f] focus:ring-2 focus:ring-[#2f4f4f]/20">
                </div>
                <div>
                    <label class="mb-1 block text-[11px] font-semibold uppercase text-slate-500">Status</label>
                    <select name="status" onchange="this.form.requestSubmit()" class="h-10 rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-700 shadow-sm focus:border-[#2f4f4f] focus:ring-2 focus:ring-[#2f4f4f]/20">
                        <option value="">All</option>
                        @foreach (['pending', 'approved', 'declined', 'disbursed'] as $rowStatus)
                ...
                <div>
                    ...
                    <select name="stage" onchange="this.form.requestSubmit()" class="h-10 rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-700 shadow-sm focus:border-[#2f4f4f] focus:ring-2 focus:ring-[#2f4f4f]/20">
                        <option value="">All</option>
                        @foreach (($stages ?? []) as $rowStage)
                            <option value="{{ $rowStage }}" @selected(($stage ?? '') === $rowStage)>{{ $rowStage }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <a href="{{ route('loan.employees.loan_applications') }}" class="inline-flex h-10 items-center rounded-lg border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50 transition-colors">Reset</a>
                <div class="ml-auto flex flex-wrap items-center gap-2">
                    <a href="{{ route('loan.employees.loan_applications', array_merge(request()->query(), ['export' => 'csv'])) }}" class="rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50">CSV</a>
                    <a href="{{ route('loan.employees.loan_applications', array_merge(request()->query(), ['export' => 'xls'])) }}" class="rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50">Excel</a>
                    <a href="{{ route('loan.employees.loan_applications', array_merge(request()->query(), ['export' => 'pdf'])) }}" class="rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50">PDF</a>
                </div>
            </div>
        </form>
        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            ...
            <form id="loan-applications-bulk-status-form" method="post" action="{{ route('loan.employees.loan_applications.bulk_status') }}" class="border-b border-slate-100 px-5 py-3" data-swal-confirm="Apply bulk status to selected applications?">
                @csrf
                <div class="flex flex-wrap items-center gap-2">
                    <select name="status" class="h-9 rounded-lg border border-slate-200 bg-white px-2 text-sm text-slate-700">
                        <option value="">Bulk status</option>
                        @foreach (['pending', 'approved', 'declined', 'disbursed'] as $rowStatus)
                            <option value="{{ $rowStatus }}">{{ ucfirst($rowStatus) }}</option>
                        @endforeach
                    </select>
            ...
            <div class="mt-3 overflow-x-auto">
                    ...
                    <thead class="bg-slate-50 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide">
                        <tr>
                            <th class="px-5 py-3"><input type="checkbox" onclick="document.querySelectorAll('.app-row').forEach(cb => cb.checked = this.checked)"></th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($applications as $app)
                            <tr class="hover:bg-slate-50/80 align-top">
                                <td class="px-5 py-3"><input type="checkbox" class="app-row" name="ids[]" value="{{ $app->id }}" form="loan-applications-bulk-status-form"></td>
