    ...
    <x-slot name="above">
        <div class="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-gray-800/80 p-4 shadow-sm">
            <div class="flex flex-wrap items-center gap-2">
                ...
                <a href="{{ route('loan.communications.bulk', array_merge((array) ($filters ?? []), ['channel' => 'sms']), absolute: false) }}" class="rounded-lg border border-emerald-300 px-3 py-1.5 text-xs font-medium text-emerald-700 hover:bg-emerald-50">SMS logs</a>
        ...
        <form method="get" action="{{ route('loan.communications.bulk') }}" class="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-gray-800/80 p-4 shadow-sm space-y-3">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <a href="{{ route('loan.communications.bulk', absolute: false) }}" class="rounded-xl border border-slate-300 dark:border-slate-600 px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700/50">Reset</a>
            </div>
        </form>

        @include('loan.communications.partials.sms_wallet_banner')

        @include('loan.communications.partials.sms_topup_card')
        ...
        <form method="post" action="{{ route('loan.communications.bulk.store') }}" class="mt-3 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-gray-800/80 p-5 shadow-sm space-y-3 max-w-2xl" x-data="{
            ...
            <div class="rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-gray-900/40 p-3">
                ...
                <div id="pm-tenant-advanced" class="mt-3 rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-gray-900 p-3 hidden space-y-2">
                    <p class="text-[11px] text-slate-500 dark:text-slate-400">Client targeting</p>
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
                        ...
                        <div>
                            <label class="block text-[11px] text-slate-600 dark:text-slate-400">Branch</label>
                            <select id="lm-client-branch" class="mt-1 w-full rounded border border-slate-200 dark:border-slate-600 bg-white dark:bg-gray-900 text-xs px-2 py-1.5" disabled>
                        ...
                        <div>
                            <label class="block text-[11px] text-slate-600 dark:text-slate-400">Client selection</label>
                    ...
                    <div id="pm-tenant-pick-wrap" class="hidden">
                        <label class="block text-[11px] text-slate-600 dark:text-slate-400">Pick clients</label>
            ...
            <div>
                ...
                <input type="text" name="segment_label" value="{{ old('segment_label') }}" required class="mt-1 w-full rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-gray-900 text-sm px-3 py-2" placeholder="e.g. All clients — Block A" />
            </div>
            <div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const clientBranchEl = document.getElementById('lm-client-branch');
    ...
    const applyClientsBtn = document.getElementById('pm-apply-clients');
    let tenantItems = [];
    ...
    function updateClientUi() {
        const branchMode = tenantScopeEl && tenantScopeEl.value === 'branch';
        if (clientBranchEl) {
            clientBranchEl.disabled = !branchMode;
            if (!branchMode) clientBranchEl.value = '';
        }
    ...
    function renderClientOptions() {
        ...
        const branch = String(clientBranchEl?.value || '');
        const opts = tenantItems.filter(item => {
            if (!branch) return true;
            return String(item.branch || '') === branch;
        });
    ...
    async function loadClientItems() {
        try {
            hint && (hint.classList.remove('hidden'), hint.textContent = 'Loading clients…');
            const channel = channelEl ? channelEl.value : 'sms';
            const branch = String(clientBranchEl?.value || '');
            let url = "{{ route('loan.communications.recipients') }}?type=clients&detailed=1&channel=" + encodeURIComponent(channel);
            if (branch) {
            ...
            if (!json.ok) {
                await showAlert(json.error || 'Failed to load client recipients.', 'Unable to load recipients', 'error');
                return;
            }
            tenantItems = Array.isArray(json.client_items) ? json.client_items : [];
            renderClientOptions();
        } catch (e) {
            console.error(e);
            await showAlert('Failed to load client recipients.', 'Unable to load recipients', 'error');
        } finally {
    ...
    async function loadPhones(type) {
        ...
        try {
            ...
            const channel = channelEl ? channelEl.value : 'sms';
            const url = "{{ route('loan.communications.recipients') }}?type=" + encodeURIComponent(type) + "&channel=" + encodeURIComponent(channel);
            const res = await fetch(url, { headers: { 'Accept': 'application/json' } });
            const json = await res.json();
            if (!json.ok) {
    ...
    clientBranchEl && clientBranchEl.addEventListener('change', async () => {
        await loadClientItems();
    });
    tenantModeEl && tenantModeEl.addEventListener('change', updateClientUi);
    applyClientsBtn && applyClientsBtn.addEventListener('click', async () => {
