...
@endphp
<div class="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-gray-800/80 p-4 shadow-sm space-y-3 w-full min-w-0">
    <div class="space-y-2">
        ...
        <div class="grid grid-cols-2 gap-2 sm:grid-cols-3 md:flex md:flex-wrap md:items-center md:gap-2">
            <a href="{{ route('loan.communications.messages', absolute: false) }}" class="{{ $quickFilterLinkClass }} border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700/50">All</a>
            <a href="{{ route('loan.communications.messages', ['period' => 'today'], absolute: false) }}" class="{{ $quickFilterLinkClass }} border-slate-300 text-slate-700 hover:bg-slate-50">Today</a>
            <a href="{{ route('loan.communications.messages', ['period' => 'week'], absolute: false) }}" class="{{ $quickFilterLinkClass }} border-slate-300 text-slate-700 hover:bg-slate-50">This week</a>
            <a href="{{ route('loan.communications.messages', ['period' => 'month'], absolute: false) }}" class="{{ $quickFilterLinkClass }} border-slate-300 text-slate-700 hover:bg-slate-50">This month</a>
            <a href="{{ route('loan.communications.messages', ['channel' => 'sms'], absolute: false) }}" class="{{ $quickFilterLinkClass }} border-emerald-300 text-emerald-700 hover:bg-emerald-50">SMS only</a>
            <a href="{{ route('loan.communications.messages', ['channel' => 'email'], absolute: false) }}" class="{{ $quickFilterLinkClass }} border-indigo-300 text-indigo-700 hover:bg-indigo-50">Email only</a>
            <a href="{{ route('loan.communications.messages', ['status' => 'failed'], absolute: false) }}" class="{{ $quickFilterLinkClass }} border-rose-300 text-rose-700 hover:bg-rose-50" title="Unresolved failures only (hides rows already sent on retry)">Failed (needs action)</a>
            <a href="{{ route('loan.communications.messages', ['has_error' => 'yes'], absolute: false) }}" class="{{ $quickFilterLinkClass }} border-amber-300 text-amber-700 hover:bg-amber-50">With errors</a>
            <a href="{{ route('loan.communications.messages', ['duplicates' => 'yes', 'status' => 'sent', 'channel' => 'sms', 'period' => 'today'], absolute: false) }}" class="{{ $quickFilterLinkClass }} border-orange-400 font-semibold text-orange-800 hover:bg-orange-50 dark:text-orange-200 dark:hover:bg-orange-950/40">Duplicates today</a>
            <a href="{{ route('loan.communications.messages', ['status' => 'failed', 'channel' => 'sms', 'period' => 'today'], absolute: false) }}" class="{{ $quickFilterLinkClass }} border-rose-400 font-semibold text-rose-800 hover:bg-rose-50 dark:text-rose-200 dark:hover:bg-rose-950/40">Failed today</a>
            <a href="{{ route('loan.communications.messages', ['status' => 'failed', 'channel' => 'sms', 'from' => '2026-06-01', 'to' => '2026-06-01'], absolute: false) }}" class="{{ $quickFilterLinkClass }} border-rose-300 text-rose-700 hover:bg-rose-50">Failed 1 Jun</a>
        </div>
    </div>
...
<form method="get" action="{{ route('loan.communications.messages') }}" class="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-gray-800/80 p-4 shadow-sm space-y-3 w-full min-w-0">
    ...
    <div class="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:items-center sm:gap-3">
        ...
        <a href="{{ route('loan.communications.messages', absolute: false) }}" class="col-span-1 w-full sm:w-auto inline-flex items-center justify-center rounded-xl border border-slate-300 dark:border-slate-600 px-4 py-2.5 sm:py-2 text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700/50 min-h-[44px] sm:min-h-0">Reset</a>
