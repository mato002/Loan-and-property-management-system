...
>
    <x-slot name="above">
        ...
        <form method="post" action="{{ route('loan.communications.templates.store') }}" class="mt-3 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-gray-800/80 p-5 shadow-sm space-y-3 max-w-2xl">
            @csrf
    ...
    <div class="space-y-2">
        ...
        <ul class="flex flex-wrap gap-2">
            @foreach ($messageTemplates as $t)
                <li>
                    <form method="post" action="{{ route('loan.communications.templates.destroy', $t) }}" data-swal-confirm="Delete template {{ $t->name }}?" class="inline">
                        @csrf
                        @method('DELETE')
