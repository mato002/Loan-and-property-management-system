...
>
    <x-slot name="above">
        @include('loan.communications.partials.communications_manage_bar', ['manageContext' => 'notifications'])

        <div class="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-gray-800/80 p-4 shadow-sm">
            <div class="flex flex-wrap items-center gap-2">
                <a href="{{ route('loan.communications.notifications', absolute: false) }}" class="rounded-lg border border-slate-300 dark:border-slate-600 px-3 py-1.5 text-xs font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700/50">Refresh</a>
                <a href="{{ route('loan.communications.messages', absolute: false) }}" class="rounded-lg border border-indigo-300 px-3 py-1.5 text-xs font-medium text-indigo-700 hover:bg-indigo-50">Open communications logs</a>
            </div>
        </div>
    </x-slot>
    ...
    <div class="rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden bg-white dark:bg-gray-800/80 shadow-sm">
        <form method="post" action="{{ route('loan.communications.notifications.bulk', absolute: false) }}">
            @csrf
                            ...
                            <tr class="border-t border-slate-100 dark:border-slate-700/80">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <div class="flex flex-wrap gap-1">
                                        <a href="{{ route('loan.communications.notifications.show', $log, absolute: false) }}" class="rounded border border-indigo-300 px-2 py-1 text-xs text-indigo-700 hover:bg-indigo-50">View</a>
                                        @if (! ($readLookup ?? collect())->has((int) $log->id))
    ...
    @foreach (($logs ?? collect()) as $log)
        @if (! ($readLookup ?? collect())->has((int) $log->id))
            <form id="notif-read-{{ $log->id }}" method="post" action="{{ route('loan.communications.notifications.bulk', absolute: false) }}" class="hidden">
                @csrf
