...
>
    <x-slot name="above">
        @include('loan.communications.partials.communications_manage_bar', ['manageContext' => 'provider'])
        ...
        <form method="get" action="{{ route('loan.communications.sms_provider', absolute: false) }}" class="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-gray-800/80 p-4 shadow-sm space-y-3">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <a href="{{ route('loan.communications.sms_provider', absolute: false) }}" class="rounded-lg border border-slate-300 dark:border-slate-600 px-3 py-1.5 text-xs font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700/50">Reset</a>
            </div>
        </form>
    </x-slot>
        ...
        @if ($lastPage > 1)
            <div class="mt-4 flex flex-wrap items-center justify-between gap-3 text-xs text-slate-600 dark:text-slate-300">
                ...
                <div class="flex flex-wrap gap-2">
                    @if ($currentPage > 1)
                        <a href="{{ route('loan.communications.sms_provider', array_merge((array) ($filters ?? []), ['page' => $currentPage - 1]), absolute: false) }}" class="rounded-lg border border-slate-300 dark:border-slate-600 px-3 py-1.5 font-medium hover:bg-slate-50 dark:hover:bg-slate-700/50">Previous</a>
                    @endif
                    @if ($currentPage < $lastPage)
                        <a href="{{ route('loan.communications.sms_provider', array_merge((array) ($filters ?? []), ['page' => $currentPage + 1]), absolute: false) }}" class="rounded-lg border border-slate-300 dark:border-slate-600 px-3 py-1.5 font-medium hover:bg-slate-50 dark:hover:bg-slate-700/50">Next</a>
                    @endif
                </div>
            </div>
        @endif
