...
<x-loan.page
    title="SMS / email"
    subtitle="Outbound SMS and email delivery log (clients and staff sends). System alerts such as logins are on Notifications."
...
>
    <x-slot name="above">
        ...
        <summary class="inline-flex cursor-pointer list-none items-center gap-2 rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700">
            ...
            <span class="hidden group-open:inline">Hide message form</span>
        </summary>
        <form method="post" action="{{ route('loan.communications.messages.store') }}" class="mt-3 rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-gray-800/80 p-5 shadow-sm space-y-3 max-w-2xl" x-data="{
            ...
            <div class="rounded-xl border border-slate-200 dark:border-slate-700 p-3">
                ...
                <div x-show="pickerOpen" x-cloak class="mt-2">
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <button type="button" @click="groupFilter = 'Client'" :class="groupFilter === 'Client' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-700 border-slate-300'" class="rounded-lg border px-2.5 py-1 text-xs font-medium">Clients</button>
            ...
            <div x-show="channel === 'email'" x-cloak>
                ...
                <input type="text" name="subject" x-model="subjectText" value="{{ old('subject') }}" class="mt-1 w-full rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-gray-900 text-sm px-3 py-2" placeholder="Email subject line" />
            </div>
            <div>
