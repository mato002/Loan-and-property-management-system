    ...
    <x-loan.page :title="$title" :subtitle="$subtitle">
        ...
        <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b border-slate-100 flex justify-between items-center">
                <h2 class="text-sm font-semibold text-slate-700">All branches</h2>
                <p class="text-xs text-slate-500">{{ $branches->total() }} record(s)</p>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($branches as $row)
                            <tr class="hover:bg-slate-50/80">
                                ...
                                <td class="px-5 py-3 text-slate-600">{{ $row->region?->name ?? '—' }}</td>
                                <td class="px-5 py-3 text-slate-600 font-mono text-xs">{{ $row->code ?? '—' }}</td>
                                <td class="px-5 py-3 text-slate-600 tabular-nums">{{ $row->loans_count }}</td>
                                <td class="px-5 py-3">
                                    @if ($row->is_active)
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-5 py-12 text-center text-slate-500">
                                    @if (\App\Models\LoanRegion::query()->exists())
                                        <a href="{{ route('loan.branches.create') }}" class="text-indigo-600 font-medium hover:underline">Add the first branch</a>.
                                    @else
                                        Create a <a href="{{ route('loan.regions.create') }}" class="text-indigo-600 font-medium hover:underline">region</a> first, then add branches.
                                    @endif
                                </td>
                            </tr>
                        @endforelse
            ...
            @if ($branches->hasPages())
                <div class="px-5 py-4 border-t border-slate-100">{{ $branches->links() }}</div>
            @endif
        </div>
    </x-loan.page>
</x-loan-layout>
