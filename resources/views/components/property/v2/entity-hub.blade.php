<div {{ $attributes->merge(['class' => 'property-entity-hub print-hide space-y-3 mb-4 sm:mb-5']) }} data-property-entity-hub="{{ $entity }}" data-property-mobile-sticky-tabs>
    @if ($alerts !== [])
                ...
                @foreach ($quickActions as $action)
                    @php
                        ...
                        $actionHref = ! empty($action['href'])
                            ? (string) $action['href']
                            : route($action['route'], $action['params'] ?? [], false);
                    @endphp
                    <a
                        href="{{ $actionHref }}"
