...
<div {{ $modalShellBag->merge(['class' => $pageClasses]) }}>
    <header class="space-y-4">
        <div class="flex flex-wrap items-start gap-x-4 gap-y-3">
            ...
            @isset($actions)
                @if (! $actions->isEmpty())
                    <div class="print-hide flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-2 w-full min-w-0 flex-[1_1_24rem] sm:w-auto sm:max-w-full sm:ml-auto [&>button]:w-full [&>button]:sm:w-auto">
                        {{ $actions }}
                    </div>
                @endif
            @endisset
