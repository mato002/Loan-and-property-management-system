...
<head>
    @php
        $companyName = \App\Models\PropertyPortalSetting::getValue('company_name', '') ?: 'PrimeEstate';
        ...
        $contactEmailPrimary = \App\Models\PropertyPortalSetting::getValue('contact_email_primary', '') ?: 'hello@primeestate.com';
        $contactPhone = \App\Models\PropertyPortalSetting::getValue('contact_phone', '') ?: '+1 (800) 555-0199';
        $contactWhatsapp = \App\Models\PropertyPortalSetting::getValue('contact_whatsapp', '') ?: '+18005550199';
        $contactAddress = \App\Models\PropertyPortalSetting::getValue('contact_address', '') ?: "123 Estate Blvd, Suite 400\nMetropolis, NY 10012";
...
<body class="font-sans antialiased text-gray-900 bg-white" x-data="{ mobileMenuOpen: false, scrolled: false }" @scroll.window="scrolled = (window.pageYOffset > 20)">
    ...
    <div class="fixed z-50 right-4 bottom-4 flex flex-col gap-3">
        <a href="https://wa.me/{{ $whatsAppDigits }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-emerald-500 hover:bg-emerald-600 text-white shadow-lg shadow-emerald-500/30" aria-label="Chat on WhatsApp">
        ...
        <a href="tel:{{ $phoneHref }}" class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-600/30" aria-label="Call agent">
    ...
    <footer class="bg-gray-900 pt-16 pb-8 border-t border-gray-800 mt-20">
        <div class="w-full px-4 sm:px-6 lg:px-12 xl:px-16 2xl:px-20">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-12 lg:gap-8">
                ...
                <div>
                    ...
                    <ul class="space-y-3 text-sm text-gray-400">
                        ...
                        <li class="text-xs text-gray-500">Reg No: {{ $contactRegNo }}</li>
                    </ul>
                </div>
            </div>
            <div class="mt-12 pt-8 border-t border-gray-800 text-sm text-gray-500 flex flex-col md:flex-row justify-between items-center">
                <p>&copy; {{ date('Y') }} {{ $companyName }} Management. All rights reserved.</p>
