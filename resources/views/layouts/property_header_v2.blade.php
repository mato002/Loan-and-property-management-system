@php
    ...
    $quickLinks = match ($portalRole) {
        ...
        default => match ($agentNavMode) {
            ...
            default => [
                ...
                ['label' => 'Revenue', 'route' => 'property.revenue.index', 'patterns' => ['property.revenue.*']],
    ...
    $agentWorkspaceLinks = $portalRole === 'agent' ? PropertyNavigation::agentHeaderWorkspaces() : [];
    $currentRouteName = Route::currentRouteName() ?? '';
    $activeAgentWorkspace = ($portalRole === 'agent' && in_array($agentNavMode, [PropertyNavMode::WORKSPACE, PropertyNavMode::HYBRID], true))
        ? PropertyNavigation::workspaceForRoute($currentRouteName)
        : null;
...
<header class="property-topbar relative z-[5000] overflow-visible flex-shrink-0 shadow-md shadow-emerald-950/10">
    ...
    <div class="bg-gradient-to-r from-emerald-700 via-emerald-600 to-emerald-700 text-white overflow-visible">
        <div class="relative z-[120] flex items-center h-14 sm:h-[60px] lg:h-[64px] px-3 sm:px-5 lg:px-6 gap-2 sm:gap-3 min-w-0">
            <div class="flex items-center gap-2 sm:gap-4 min-w-0 shrink">
                <button
                    ...
                    class="property-mobile-sidebar-toggle lg:hidden shrink-0 p-2 rounded-lg text-white/90 hover:bg-white/15 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-white/60"
                ...
                @isset($header)
                    <div class="hidden md:flex items-center min-w-0 pl-3 sm:pl-4 ml-2 sm:ml-3 border-l border-white/25">
                        @if ($portalRole === 'agent' && $activeAgentWorkspace)
            ...
            @if ($portalRole === 'agent')
                ...
                >
                    ...
                    <div class="relative w-full z-[150]" id="property-global-search-wrap">
                        ...
                        <div id="property-search-suggest" class="hidden pointer-events-none absolute z-[99999] mt-2 w-full rounded-xl border border-slate-200 bg-white text-slate-800 shadow-2xl overflow-hidden"></div>
                    </div>
                </form>
            @endif
        ...
        @if ($portalRole === 'agent' && $agentWorkspaceLinks !== [])
            ...
            >
                <nav class="property-header-workspaces flex flex-wrap items-center gap-1 px-4 py-2" aria-label="Workspaces">
                    @foreach ($agentWorkspaceLinks as $link)
        ...
        @elseif ($portalRole !== 'agent')
            {{-- Landlord / tenant: compact portal nav on md+ only (no mobile horizontal scroll) --}}
            <div class="relative z-[20] hidden md:block border-t border-white/15 bg-emerald-800/40 backdrop-blur-sm">
                <nav class="property-header-workspaces flex flex-wrap items-center gap-1 px-4 py-2" aria-label="Portal sections">
...
@if ($portalRole === 'agent')
    <script>
        function initPropertyHeaderLiveSearch() {
            ...
            const closeBox = () => {
                box.classList.add('hidden', 'pointer-events-none');
                box.innerHTML = '';
                firstUrl = '';
            };
            const render = (payload, q) => {
