...
<aside
    class="property-sidebar fixed inset-y-0 left-0 z-[5600] lg:z-50 h-screen w-[min(100vw-2.5rem,300px)] sm:w-[312px] bg-[#2f4f4f] border-r border-[#264040] text-[#d4e4e3] text-base transform transition-[transform,width] duration-300 ease-out lg:translate-x-0 lg:static lg:inset-0 flex flex-col min-h-0 shadow-xl shadow-black/20 lg:shadow-none overflow-x-hidden flex-shrink-0"
...
>
    ...
    <div class="property-sidebar-collapse-toggle-wrap hidden lg:flex justify-end px-3 pt-2 shrink-0">
        <button
            type="button"
    ...
    <nav class="flex-1 min-h-0 overflow-y-auto overscroll-contain py-4 px-2.5 custom-scrollbar">
        ...
        @if (auth()->check() && (auth()->user()->is_super_admin ?? false))
            <a
                ...
                class="mb-3 flex items-center gap-3 rounded-xl border border-[#406866]/60 bg-[#243d3d]/35 px-3 py-2.5 text-sm font-semibold text-white hover:bg-[#406866]/50 transition-colors property-collapse-center property-collapse-compact"
                :title="sidebarDesktopOpen ? '' : 'Super Admin'"
            >
                <svg class="h-5 w-5 shrink-0 text-[#c5ebe8]" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
        ...
        <a
            ...
            class="property-sidebar-brand mb-3 flex items-center gap-3 rounded-xl border border-[#406866]/60 bg-[#243d3d]/35 px-3 py-2.5 property-collapse-center property-collapse-compact"
            :title="sidebarDesktopOpen ? '' : '{{ $companyName !== '' ? $companyName : 'Home' }}'"
        >
            <span class="property-sidebar-brand-logo flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[#406866]/60 ring-1 ring-[#5a8583]/50">
                @if ($companyLogoUrl)
    ...
    <div class="p-3 border-t border-[#264040] bg-[#243d3d]/40 shrink-0">
        ...
        >
            <div class="property-sidebar-avatar w-11 h-11 rounded-full bg-teal-500/25 border border-teal-400/35 flex items-center justify-center text-teal-200 font-semibold text-base shrink-0">
