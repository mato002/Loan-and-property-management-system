    ...
    <nav class="flex-1 overflow-y-auto py-4 px-2.5 space-y-1 custom-scrollbar">
        @foreach ($menu as $itemName => $data)
            ...
            <a
                ...
                class="group flex items-center gap-3 px-3 py-3 rounded-xl text-base font-medium border-l-[3px] transition-all duration-150 {{ $active ? 'border-teal-300 bg-[#406866]/80 text-white font-semibold' : 'border-transparent text-[#d4e4e3] hover:bg-[#406866]/50 hover:text-white' }}"
            >
                <svg class="w-6 h-6 shrink-0 {{ $active ? 'text-teal-200' : 'text-[#8db1af] group-hover:text-white' }} transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
